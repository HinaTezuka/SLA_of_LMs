...
# activations
activations = np.zeros((32, 14336, 5000))
for L2 in langs:
    start_idx = start_indics[L2]
    end_idx = start_idx + 1000
    save_path_activations = f"/home/s2410121/proj_LA/activated_neuron/new_neurons/pickles/transfer_neurons/aya/activations/{L2}_activations_crr_ratio.npz"
    activations_arr = unfreeze_np_arrays(save_path_activations)
    activations[:, :, start_idx:end_idx] = activations_arr
...
for L2 in langs:
    for score_type in score_types:
        ...
        for (layer_i, neuron_i) in sorted_neurons[:top_n]:
            corr_ratio = correlationRatio(labels_list, activations[layer_i, neuron_i, :])
