...
def track_neurons_with_text_data(model, model_name, device, tokenizer, data, active_THRESHOLD=0.01):
    ...
    for L1_text, L2_text in data:
        ...
        for layer_idx in range(len(mlp_activation_L2)):
            ...
            # act_sum_shared
            for neuron_idx in shared_neurons_layer:
                act_value_L1 = get_activation_value_abs(mlp_activation_L1, layer_idx, neuron_idx, token_len_L1-1)
                act_value_L2 = get_activation_value_abs(mlp_activation_L2, layer_idx, neuron_idx, token_len_L2-1)
    ...
    return output_dict_vis, freq_dict, act_sum_dict

def get_activation_value_abs(mlp_activations, layer_idx, neuron_idx, token_idx):
    """
    ...
    """
    activation_value = abs(mlp_activations[layer_idx][0, token_idx, neuron_idx].item())
