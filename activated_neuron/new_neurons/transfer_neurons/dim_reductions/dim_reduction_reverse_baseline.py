...
def plot_pca(model_type: str, features_L1: dict, features_L2: dict, features_L3: dict, features_L4: dict, features_L5: dict, is_reverse: bool):
    ...
    for layer_idx in range(start, end):  # Embedding layer + 32 hidden layers
        ...
        # save as image.
        if is_reverse:
            output_dir = f'/home/s2410121/proj_LA/activated_neuron/new_neurons/images/transfers/dim_reduction/{model_type}/reverse/baseline'
        else:
        ...
        for L2 in langs:
            ...
            if L2 != "en":
                if is_reverse: # type-2
                    ...
                    sorted_neurons = unfreeze_pickle(save_path_sorted_neurons)
                sorted_neurons_main = sorted_neurons[:1000]
                random.seed(42)
                sorted_neurons_baseline = random.sample(sorted_neurons[1000:], 1000)
