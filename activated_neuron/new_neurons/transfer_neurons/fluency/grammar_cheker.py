...
for model_type in model_types:
    for L2 in langs:
        tool = language_tool_python.LanguageTool(L2)
        for is_baseline in is_baselines:
            ...
            for sentence_i, sentence in enumerate(sentences):
                ...
                if pred_language is not None and pred_language.is_reliable and pred_language.language == 'ja':
                    matches = tool.check(sentence)
                    if len(matches) == 0: cnt += 1
            """ print results. """
