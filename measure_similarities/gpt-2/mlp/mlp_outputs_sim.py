...
def get_out_gpt2_mlp(model, prompt, device):
  ...
  MLP_values = [f"transformer.h.{i}.mlp" for i in range(num_layers)]  # generate path to MLP layer(of LLaMA-3)
  ...
  with torch.no_grad():
      ...
      MLP_value = [ret[act_value].output for act_value in MLP_values]  # get outputs of self_att per layer
      return MLP_value

def get_outputs_mlp(model, input_ids):
    MLP_values = get_out_gpt2_mlp(model, input_ids, model.device)  # Llamaのself-att直後の値を取得
...
def plot_hist(dict1: defaultdict(float), dict2: defaultdict(float), L2: str) -> None:
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f"/home/s2410121/proj_LA/measure_similarities/gpt-2/images/mlp_outputs_sim/cos_sim/en_{L2}.png")
    plt.close()
